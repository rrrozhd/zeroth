...
- [x] Write RED tests that open two SQLite database instances against one file, hold `transaction(write_lock=True)` in the first, and prove the second cannot enter its critical section until release.
- [x] Add `backend: Literal["sqlite", "postgres"]` and `transaction(*, write_lock: bool = False)` to `AsyncDatabase`.
- [x] SQLite: execute `BEGIN IMMEDIATE` before yielding when `write_lock=True`.
...
- [x] Postgres: expose `backend = "postgres"` and, for `write_lock=True`, execute
...
- [x] Run GREEN: `uv run pytest -q tests/storage/test_coordination.py tests/storage/test_sqlite.py`.
- [x] Commit as `feat: add database coordination transactions`.
...
- [x] Write RED migration tests for `audit_chain_heads(run_id PK, head_digest,
...
- [x] Implement `ensure_and_lock_row(connection, *, backend, table, key_column, key)`:
...
- [x] Add a Postgres contention test in which one transaction holds a coordination
...
- [x] Run the SQLite migration/locking tests and the Postgres contention test when
...
- [x] Write RED test using two `AuditRepository` instances and `asyncio.gather` to
...
- [x] Add `NodeAuditRecord.chain_sequence: int | None = None` and include it in the
...
- [x] In a `transaction(write_lock=True)`, lock/initialize the run head, lazily derive
...
- [x] Order per-run audit reads and verification by `chain_sequence`, falling back to
  ...
  order.
- [x] Keep the local `asyncio.Lock` only as a same-process contention optimization; instantiate it per repository only if tests prove no correctness dependency.
- [x] Add failure-injection test: duplicate audit ID rolls back head advancement and the next valid write chains from the prior head.
- [x] Run GREEN: `uv run pytest -q tests/audit/test_audit_concurrency.py tests/audit`.
- [x] Add a marked Postgres version using two pooled connections; run when Docker is available.
- [x] Commit as `fix: coordinate audit chains across workers`.
...
- [x] Write RED tests proving run-specific and tenant-wide hold operations contend on the same tenant row.
- [x] Implement `RetentionCoordinator.transaction(tenant_id)` using `database.transaction(write_lock=True)` and `ensure_and_lock_row(... retention_coordination ...)`.
- [x] Add connection-aware `place_in_transaction`, `release_in_transaction`, and `active_holds_for_tenant_in_transaction`; public methods open the coordinator transaction.
- [x] Do not add a second run-level lock.
- [x] Run GREEN and commit as `feat: serialize tenant retention administration`.
...
- [x] Write a barrier-based RED test: pause erasure after lock acquisition, attempt hold placement from a second service/repository instance, then prove deterministic serialization and no stale hold check.
- [x] Add connection-aware audit tombstone, checkpoint delete, run redact, and
...
- [x] Under the tenant coordinator transaction: re-read holds, reject if blocked,
...
- [x] After commit: perform artifact/econ cleanup and append completion/failure log
...
- [x] Add a RED/GREEN failure-injection test: external cleanup fails after database
...
- [x] Run GREEN: `uv run pytest -q tests/retention/test_coordination.py tests/retention/test_erasure_service.py`.
- [x] Commit as `fix: make legal holds and erasure race-free`.

### Task 6: Coordination verification

- [x] Run `uv run pytest -q tests/storage tests/audit tests/retention`.
- [x] Run Postgres marked tests with `uv run pytest -q -m postgres tests/storage/test_coordination.py tests/audit/test_audit_concurrency.py` when Docker is available.
- [x] Run `uv run alembic upgrade 010`, downgrade to 008, then upgrade to head on disposable SQLite and Postgres databases.
- [x] Record any unavailable Postgres verification explicitly; do not silently claim it passed.
