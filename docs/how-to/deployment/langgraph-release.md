...
This is the canonical clean install and operations path for Zeroth `0.25.9.11.1`.
...
Use Python 3.12 and install only the deployment surface you operate, from a
checkout at the release tag (PyPI currently serves a stale `0.1.0` placeholder,
so the documented version is installed from source):

```bash
git clone https://github.com/rrrozhd/zeroth.git
cd zeroth
git checkout v0.25.9.11.1
