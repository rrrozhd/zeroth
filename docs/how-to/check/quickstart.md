...
zeroth check explain .zeroth/check/payment-report/check-verdict.json
```

To create a tape for your own target:

```bash
zeroth check record --config zeroth-check.yaml --case payment-7 \
  --allow-side-effects
zeroth check curate .zeroth/check/recordings/payment-7-payment-7-baseline.json \
