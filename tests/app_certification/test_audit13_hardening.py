...
def _replaced_runtime_inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path, Path]:
    ...
    wheel = tmp_path / "zeroth_platform-0.23.9.16-py3-none-any.whl"
...
def test_replaced_candidate_runtime_is_excluded_from_certifier_context(tmp_path: Path) -> None:
    ...
    assert (
        "COPY zeroth_platform-0.23.9.16-py3-none-any.whl "
        "/opt/zeroth/zeroth_platform-0.23.9.16-py3-none-any.whl"
    ) in dockerfile
    assert "/opt/zeroth/zeroth_platform-0.23.9.16-py3-none-any.whl" in dockerfile
