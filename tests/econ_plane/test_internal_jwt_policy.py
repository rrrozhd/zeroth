"""Internal token policy on PyJWT: library defaults plus refusal of binding claims."""
...
def test_internal_tokens_reject_future_issued_at(token):
    with pytest.raises(jwt.InvalidTokenError):
        service.decode_token(token(iat=int(time.time()) + 3600))


def test_internal_tokens_accept_current_issued_at_and_scope(token):
    claims = service.decode_token(token(iat=int(time.time()) - 5))
