...
def test_model_migration_route_retains_an_immutable_tenant_decision(
    ...
    assert payload["verdict"] == "recommend"
    assert payload["recommended_action"] == "ship_candidate"
    ...
    assert assignment.json()["assigned_model"] in {"model-a", "model-b"}
    assert verification.status_code == 200, verification.text
    assert verification.json()["causal_status"] == "inconclusive"
