...
async def test_sqlite_repository_reconstruction_preserves_scope_predicates(tmp_path: Path) -> None:
    ...
    try:
        owner_runs = RunRepository(restarted, NullWorkspaceScopeContext(tenant_id="tenant-a"))
        foreign_runs = RunRepository(restarted, NullWorkspaceScopeContext(tenant_id="tenant-b"))
        foreign_threads = ThreadRepository(
            restarted, NullWorkspaceScopeContext(tenant_id="tenant-b")
        )
        assert await foreign_runs.get("owner-run") is None
        assert await foreign_runs.get("unknown-run") is None
        assert await owner_runs.get("owner-run") is not None
        assert await foreign_threads.get("owner-thread") is None
        assert await foreign_threads.get("unknown-thread") is None
        assert (
...
async def test_execution_result_guessing_stays_hidden_after_restart(tmp_path: Path) -> None:
    ...
    await RunRepository(first, NullWorkspaceScopeContext(tenant_id="tenant-a")).create(owner)
    await first.close()

    restarted = AsyncSQLiteDatabase(str(database_path))
    try:
        owner_repository = RunRepository(restarted, NullWorkspaceScopeContext(tenant_id="tenant-a"))
        foreign_repository = RunRepository(
            restarted, NullWorkspaceScopeContext(tenant_id="tenant-b")
        )
        foreign = await foreign_repository.get("result-run")
        unknown = await foreign_repository.get("unknown-run")
        assert foreign is unknown is None
        assert (await owner_repository.get("result-run")).final_output == {
            "secret_result": "tenant-a-only"
        }
    finally:
        await restarted.close()
...
async def test_checkpoint_guessing_is_hidden_by_owning_run_tenant(sqlite_db) -> None:
    owner_repository = RunRepository(sqlite_db, NullWorkspaceScopeContext(tenant_id="tenant-a"))
    foreign_repository = RunRepository(sqlite_db, NullWorkspaceScopeContext(tenant_id="tenant-b"))
    ...
    foreign = await foreign_repository.get_checkpoint(owner.checkpoint_id)
    unknown = await foreign_repository.get_checkpoint("unknown-checkpoint")

    assert foreign is unknown is None
    assert await owner_repository.get_checkpoint(owner.checkpoint_id) is not None


async def test_worker_claims_only_its_tenant_pending_runs(sqlite_db) -> None:
    tenant_a = RunRepository(sqlite_db, NullWorkspaceScopeContext(tenant_id="tenant-a"))
    tenant_b = RunRepository(sqlite_db, NullWorkspaceScopeContext(tenant_id="tenant-b"))
    await tenant_a.create(
        _run(
            "tenant-a-run",
            tenant_id="tenant-a",
    ...
    foreign = await tenant_b.get("tenant-b-run")
...
async def test_restarted_dispatch_worker_executes_only_its_deployment_tenant(
    ...
    for tenant in ("tenant-a", "tenant-b"):
        await RunRepository(first, NullWorkspaceScopeContext(tenant_id=tenant)).create(
            _run(
                f"{tenant}-worker-run",
                tenant_id=tenant,
    ...
    restarted = AsyncSQLiteDatabase(str(database_path))
    repository = RunRepository(restarted, NullWorkspaceScopeContext(tenant_id="tenant-a"))
    ...
    tenant_b = await RunRepository(restarted, NullWorkspaceScopeContext(tenant_id="tenant-b")).get(
        "tenant-b-worker-run"
    )
