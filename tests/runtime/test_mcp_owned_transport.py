...
@pytest.mark.asyncio
async def test_repeated_cancellation_does_not_interrupt_owned_removal(monkeypatch) -> None:
    ...
    monkeypatch.setattr(transport, "_control", control)
    task = asyncio.create_task(hold_transport_open())
    await asyncio.sleep(0)
    task.cancel()
    await entered.wait()
    try:
        # A second cancellation arriving mid-removal must not abandon the container.
        task.cancel()
        await asyncio.sleep(0)
