...
- The documentation reference gate reports actionable startup instructions
  while preserving explicit version-history examples.
...
- Historical documentation markers bind only to the nearest inline code span,
  keeping unrelated replacement targets actionable.
...
- A slow wait is now recorded durably, not just warned about: `wait_for` appends elapsed
  time, poll count, threshold and the observed run state to an ignored local JSONL file
...
- README and SECURITY.md updated so product claims
