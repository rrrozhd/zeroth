...
- RegulusClient: thin wrapper around the instrumentation SDK client
...
if TYPE_CHECKING:
    ...
    from zeroth.econ.analytics.quality import (
        QualityEconomicsReport,
        RunQualityVerdict,
        quality_economics,
        read_quality_verdict,
    )
    from zeroth.econ.analytics.rightsizing import (
        ModelOption,
        RightsizingResult,
        describe,
        recommend,
...
def __getattr__(name: str) -> object:
    ...
    package. Caching the function over it reproduces what an eager
    ``from ... import unit_economics`` would do, so
    ``zeroth.econ.analytics.unit_economics`` keeps resolving to the callable.
    """
...
#
# The legacy package used to resolve this by importing every submodule eagerly,
# so the ``from ... import unit_economics`` line in this file ran last and won.
# Exports are lazy now -- eagerly importing this submodule would pull the run
# domain into the platform layer -- so the collision is resolved explicitly
# instead.
