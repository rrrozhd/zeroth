    ...
    async def escalate(
        ...
        *,
        ...
        if action == "delegate":
            ...
            # Resolving REJECT alone leaves the run parked in WAITING_APPROVAL.
            # schedule_continuation's REJECT branch marks the run FAILED
            # (reason='approval_rejected') and records the decision audit, so the
            # SLA rejection actually terminates the run instead of wedging it.
