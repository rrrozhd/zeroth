...
These models are defined here so runtime code depends on the run domain rather
than on whichever package happens to persist it.

These Pydantic models are what gets saved to the database when a workflow runs.
They track which nodes have executed, what happened at each step, and the
...
# Re-exported as zeroth.runtime.runs API: the conditions contracts own the
...
from zeroth.contracts.governed import (
