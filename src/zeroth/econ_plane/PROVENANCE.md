# Provenance: zeroth.econ_plane (economic control plane)
...
- Its own JWT bearer auth (`zeroth.econ_plane.auth`). Tokens are minted via the open
...
```bash
uv run uvicorn zeroth.econ_plane.main:app --port 8000
```
