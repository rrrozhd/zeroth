...
class SandboxSidecarClient:
    ...
    async def close(self) -> None:
        """Refuse new requests and close any explicitly injected HTTP client."""
        self._closed = True
        if self._client is not None:
            await self._client.aclose()

    async def _request(self, method: str, url: str, **kwargs: Any) -> httpx.Response:
        """Own transport connections until this request finishes on its caller's loop."""
        # Tests may inject a transport on an instance built without __init__.
        ...
        # A request owns its cache: sync bridges cannot inherit another loop's
        # sockets, and waiting executions cannot consume cancellation capacity.
        ...
        try:
            ...
            await cleanup_despite_cancellation(aclose_all(owner))
