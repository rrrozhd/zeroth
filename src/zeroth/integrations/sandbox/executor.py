...
class SidecarExecutor:
    ...
    async def _await_shielded_task(
        ...
        A propagated cancellation leaves the task's cancellation count intact so
        the requester (``asyncio.timeout``, a task group) still recognises it; a
        swallowed one is uncancelled once per held-back request instead.
        """
        _, cancellations = await finish_despite_cancellation(task)
