"""Legacy import path for the gateway endpoint inventory.

The endpoint inventory now live in :mod:`zeroth.contracts.langgraph_gateway.inventory`; this
module republishes exactly the names it published before ZER-24 relocated it.
Import from the canonical location instead (see docs/backend-import-migration.md).

Resolution is lazy on purpose: importing this shim must not drag the contracts
package onto the import path of anything that merely touches
``zeroth.core.langgraph_gateway``.

The export list is the surface captured *before* the move, in
``tests/langgraph_gateway/fixtures/legacy_surface_manifest.json``. It is wider
than a curated ``__all__`` because this module never declared one -- its public
surface was whatever a star-import saw, incidental imports included -- and
narrowing it now would break a caller that relied on the old behaviour.
...
def __getattr__(name: str) -> Any:
    ...
    import zeroth.contracts.langgraph_gateway.inventory as _canonical
