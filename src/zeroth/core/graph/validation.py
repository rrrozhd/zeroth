...
class GraphValidator:
    ...
    async def validate(self, graph: Graph) -> GraphValidationReport:
        ...
        self._validate_join_configs(graph, node_map, adjacency, issues)
    ...
    def _validate_join_configs(
        ...
        * **JOIN_ON_CYCLE** — a convergent node (>=2 non-tool inbound edges) that
        ...
        for component in self._strongly_connected_components(node_map.keys(), adjacency):
            if len(component) > 1:
